...
def find_centre_coords(md, TT, NN):
    filepath = "/nfs/a37/scjea/model_runs/Hagupit/u-af761/data/track_data/numpy_data/std1/track_data_{0:04d}{1:02d}00.npy.npz".format(
        md, TT)
...
def find_centre_coords_numpy(md, TT, NN):
    filepath = "/nfs/a37/scjea/plot_scripts/tc_analysis/intensity_track/track_data/{0:04d}_{1:02d}.npz".format(
        md, TT)
    track_data = np.load(filepath)
    lats = track_data['arr_0']
    lons = track_data['arr_1']
...
def find_centre_ens_glm(md, TT, em, NN):
    filepath = "/nfs/a37/scjea/plot_scripts/tc_analysis/intensity_track/track_data/tcver_glm_{0:04d}_{1:02d}Z_en{2:02d}.npz".format(
        md, TT, em)
    track_data = np.load(filepath)
    lats = track_data['lats']
    lons = track_data['lons']
...
def find_centre_ens(md, TT, em, NN, mod):
    filepath = "/nfs/a37/scjea/plot_scripts/tc_analysis/intensity_track/track_data/tcver_{3}_{0:04d}_{1:02d}Z_en{2:02d}.npz".format(
        md, TT, em, mod)
    track_data = np.load(filepath)
    lats = track_data['lats']
    lons = track_data['lons']
...
def find_centre_3h(md, TT, em, v_day, v_time, mod):
    ...
    # elif mod == 'glm':
    #
    df = "/nfs/a319/scjea/old_trackdata/orig_pctrack_firsttime_{0}_{1:02d}Z_{2}_em{3:02d}.npy".format(
        md, TT, mod, em)
...
def load_tcverdata_3h_df(df0, storm, mod, md, TT):
    ...
    for em in ens_members:
        ...
        if storm == 'hagupit':
            ...
            print('Storm unknown')
            exit()
        tcver_lats = data['lats']
        # tcver_press = data['minpress']; tcver_maxws = data['max_ws']
        tcver_lons = data['lons']
        tcver_days = data['vt_days']
        tcver_hrs = data['vt_times']
        dates = []
        ...
        if i == 0:
            lats = tcver_lats
            lons = tcver_lons  # press = tcver_press; maxws = tcver_maxws
            vts = dates
            i = i + 1
        else:
            lats = np.vstack([lats, tcver_lats])
            # press = np.vstack([press,tcver_press]); maxws = np.vstack([maxws,tcver_maxws])
            lons = np.vstack([lons, tcver_lons])
...
def track_data(filename):
    ...
    times = num2date(dataset.variables['time_wmo']
                     [:], dataset.variables['time_wmo'].units)
