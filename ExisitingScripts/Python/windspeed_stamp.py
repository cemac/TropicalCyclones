...
def main():
    ...
    data_name = 'uvplev_4p4_1203_12Z_em' # Should include everything but 'NN.pp' where NN is the ensemble member.
    outfile_loc = '/nfs/a319/scjea/figures/'
    outfile_name = 'wspeed_1203_12Z_4p4_850hPa_' # time data to be added to the outfile 
    
    ens_members = np.arange(12) # Ensemble members to use (i.e. 12)
    plev = 850 # Must be alevel that is in the data.

    md = 1203; TT = 12 # Forecast initial times
    model = '4p4' 

    v_times = [0,3,6,9,12,15,18,21] # Times during day to plot
    init_day = 3; init_time = 12 # The first plot will be of the time after this time, i.e. in this case the 4th at 15Z
    final_day = 8; final_time = 9 # Last time to plot
    v_days = np.arange(init_day, init_day + 6, 1)
    yr = 2014; mth = 12

    domain_rad = 4. # Domain is 2*domain_rad x 2*domain_rad

    u_stash = 'm01s15i201'; v_stash = 'm01s15i202'; slp_stash = 'm01s16i222'
    ...
    for dd in v_days:
        for hr in v_times:
            ...
            outfile = outfile_loc + outfile_name + '{0:02d}_{1:02d}Z.png'.format(dd,hr)
            ...
            minlat = cenlat - domain_rad; maxlat = cenlat + domain_rad; 
            minlon = cenlon - domain_rad; maxlon = cenlon + domain_rad
            b_constraint = box_constraint(minlat,maxlat,minlon,maxlon)

            # Create figure
            fig, axs = plt.subplots(nrows,ncols,dpi=100,subplot_kw={'projection': ccrs.PlateCarree()})
            for i, em in enumerate(ens_members):
                ...
                with iris.FUTURE.context(cell_datetime_objects=True):
                    u = iris.load_cube(df,u_constraint).extract(time_constraint&p_constraint&b_constraint)
                    v = iris.load_cube(df,v_constraint).extract(time_constraint&p_constraint&b_constraint)
                ws = (u**2 + v**2)**0.5
                ...
                map_formatter(ax,bottom_label=bottom_label, left_label=left_label, labelsize=20,tick_base_x=2,tick_base_y=2)

                # Annotate to add the ensemble member onto the plot
                ax.annotate('{0:02d}'.format(em), xy=(0.97,0.03), xycoords='axes fraction',horizontalalignment='right',verticalalignment='bottom',color='k',backgroundcolor='white',fontsize=20)

            # Reduce white space and then make whole figure bigger, keeping the aspect ratio constant.    
            plt.gcf().subplots_adjust(hspace=0.025,wspace=0.025,bottom=0.05,top=0.95,left=0.075,right=0.925)
            w, h = plt.gcf().get_size_inches()
            plt.gcf().set_size_inches(w * 3, h * 3)
            ax = fig.get_axes()

            # Format colourbar
            cbar = plt.colorbar(wspeed_plt,ax=fig.get_axes(), orientation='horizontal',extend='both',fraction=0.046,pad=0.09)
            cbar.ax.tick_params(labelsize=18)
            cbar.set_label('ms$^{-1}$',size=18)
            axs = fig.gca()

            # Add initial time, valid time etc.
            axs.annotate('Initial time: {0}/{1}/{2}, {3:02d}Z'.format(str(md)[-2:],str(md)[:2],yr,TT), xy=(0.95,0.95),xycoords='figure fraction',horizontalalignment='right',verticalalignment='top',color='k',fontsize=15)
            axs.annotate('Valid time: {0:02d}/{1:02d}/{2}, {3:02d}Z'.format(dd,mth,yr,hr), xy=(0.95,0.925),xycoords='figure fraction',horizontalalignment='right',verticalalignment='top',color='k',fontsize=15)
            axs.annotate('T+{0}Z'.format(lead_time), xy=(0.95,0.9),xycoords='figure fraction',horizontalalignment='right',verticalalignment='top',color='k',fontsize=15)
...
def plot_wspeed(ax,ws):
    ...
    wspeed = ax.contourf(X,Y,ws.data,cmap=cmap,levels=levels,extend='both')
    return wspeed
...
def plot_winds(ax,u,v,mod):
    ...
    arrows = ax.quiver(X[::step, ::step],Y[::step,::step],u.data[::step,::step],v.data[::step,::step], angles='xy', scale=scale, transform=ccrs.PlateCarree())
    if u.coord('ensemble_member').points[0] == 0:
        ax.quiverkey(arrows,0.85,0.1,20,'20ms$^{-1}$',coordinates='figure',fontproperties={'size':15})
