...
def load_ens_members(fpath, dd, hr,plev=0, yr=2014, mth=12):
    ...
    for em in ensemble_members:
        ...
        if plev == 0:
            ...
            with iris.FUTURE.context(cell_datetime_objects=True):
                T = iris.load_cube(df,data_constraint).extract(p_constraint&time_constraint&b_constraint)
        cube_max = np.amax(T.data); cube_min = np.amin(T.data)
        if cube_max>max_val:
            max_val = cube_max
        if cube_min < min_val:
            min_val = cube_min
        T_list.append(T); 
    return T_list, min_val, max_val
...
def load_era_data(cube,dd,hr,plev,yr=2014,mth=12):
    ...
    b_constraint=box_constraint(minlat,maxlat,minlon,maxlon)
    T = iris.load_cube(df,'air_temperature').extract(p_constraint&b_constraint)[0][:][:]
    return T
    
def plot_data(T_list,avg_T,era_T,min_val,max_val,outfile,md,TT,pos_id,dtick=5):
    ...
    for em in ensemble_members:
        T = T_list[em];
        T = T - avg_T
        min_v = np.amin(T.data)
        max_v = np.amax(T.data)
        if em == 0: 
    ...
    cmap, norm, T_levs = normalise_cmap(min_val,max_val,0,dl,cmap=brewer_cmap)
    ...
    for em in ensemble_members:
        ...
        T_plt = iplt.contourf(T,levels=T_levs,cmap=cmap,norm=norm)
#        ws_plt.cmap.set_under('b'); ws_plt.cmap.set_over('r')
        ...
        map_formatter(ax,bottom_label=bottom_label, left_label=left_label, labelsize=20,tick_base_x=10,tick_base_y=10)
    ...
    colorbar = plt.colorbar(T_plt, colorbar_axes, orientation='horizontal')
    plt.suptitle('Temperature @ 850hPa')    
            ...
            for dd in vt_day:
                for hr in vt_hr:
                    if dd==(md-1200) and hr < TT:
                        continue
                    for plev in plevs:
                        outfile = '/nfs/a37/scjea/model_runs/Hagupit/u-ap087/plots/glm/{0:04d}_{1:02d}Z/ensemble_plots/temp_anom_{2:02d}_{3:02d}Z_{4:03d}hPa.png'.format(md,TT,dd,hr,plev)
                        fpath = '/nfs/a37/scjea/model_runs/Hagupit/u-ap087/data/glm/{0:04d}_{1:02d}Z/gen_diags/gendiags_glm_{0:04d}_{1:02d}Z_em'.format(md,TT)
                        T_list, min_val, max_val = load_ens_members(fpath,dd,hr,plev=plev)
                        avg_T = avg_cubes(T_list); 
                        era_T = load_era_data(avg_T,dd,hr,plev)
                        plot_data(T_list,avg_T,era_T,min_val,max_val,outfile,md,TT,pos_id)
                        pos_id = pos_id + 1
