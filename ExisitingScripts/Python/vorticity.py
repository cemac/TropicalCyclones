...
def plot_divergence(div, outfile):
    ...
    cbar = plt.colorbar(div_plt, orientation='vertical', fraction=0.046,
                        pad=0.09, format=OOMFormatter(-4, mathText=False))
...
# Load all data
    ...
    psi = iris.load_cube(filename, psi_constraint).extract(
        b_constraint & iris.Constraint(pressure=plev))
    print psi.data
