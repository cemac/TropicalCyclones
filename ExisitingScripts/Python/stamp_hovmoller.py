...
def load_ens_members(em,fpath,x0,y0):
    ...
    for u_slc, v_slc in zip(u.slices(['latitude','longitude']),v.slices(['latitude','longitude'])):
        if i < 2 :
            minlat = y0[0] -5; maxlat = y0[0] + 5; minlon = x0[0]-5; maxlon = x0[0]+5
        else:
            minlat = y0[i/2-1] -5; maxlat = y0[i/2-1] + 5; minlon = x0[i/2-1]-5; maxlon = x0[i/2-1]+5
        ...
        for r in ranges:
            ...
            for phi in phis:
                ...
                newu = u_slc.interpolate(new_point,iris.analysis.Linear()).data
                newv = v_slc.interpolate(new_point,iris.analysis.Linear()).data
                aziv = -newu*np.sin(phi) + newv*np.cos(phi)
                radu =  newu*np.cos(phi) + newv*np.sin(phi)
                if phi == 0:
                    vazi = aziv; urad = radu
                else:
                    vazi = np.append(vazi,aziv); urad = np.append(urad,radu)

            if r == 0:
                Vazi = np.mean(vazi); Urad = np.mean(urad)
            else:
                Vazi = np.append(Vazi,np.mean(vazi)); Urad = np.append(Urad,np.mean(urad))
        print i
        if i == 0:
            v_azi_all = Vazi; u_rad_all = Urad
        else:
            v_azi_all = np.dstack((v_azi_all,Vazi)); u_rad_all = np.dstack((u_rad_all,Urad))
    ...
    for md in monthday:
        for TT in times:
            ...
            for em in ensemble_members:
                outfile = '/nfs/a37/scjea/model_runs/Hagupit/u-ap087/plots/glm/{0:04d}_{1:02d}Z/ensemble_plots/hovmoller_4p4_{0:04d}_{1:02d}Z_em{2:02d}.png'.format(md,TT,em)
