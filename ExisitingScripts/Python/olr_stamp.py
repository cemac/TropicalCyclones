...
def main():
    ...
    # for i in range(numfilestoload-1):
        ...
        if i == 0:
            ...
            irtemplate = ir.extract(iris.Constraint(
                longitude=lambda cell: minlon < cell < maxlon, latitude=lambda cell: minlat < cell < maxlat))
            ir = ir.extract(iris.Constraint(longitude=lambda cell: minlon
                                            < cell < maxlon, latitude=lambda cell: minlat < cell < maxlat))
    ...
    # levelssst=[26.5,27,27.5,28,28.5,29,29.5,30,30.5,31,31.5,32]
        ...
        ax.annotate('P{0}'.format(i), xy=(0.97, 0.03), xycoords='axes fraction', horizontalalignment='right',
                    verticalalignment='bottom', color='k', backgroundcolor='white', fontsize=12)

    #ax.set_extent([-80.0, -50.0, 10.0, 35.0])
        ax.set_extent([lonn[timeselect] - 5, lonn[timeselect] + 5,
                       latt[timeselect] - 5, latt[timeselect] + 5])

    #sstcontour = ax.contourf(X,Y,sst.data,levels=sstlevels,norm=norm,cmap=cmap,extend='both')

        # print(1/0)
        print(dataarray.shape)
        vvcontour = ax.contourf(
            X, Y, dataarray, levels=levelsvv, cmap=plt.cm.binary, extend='both')
