...
def load_ens_members(fpath, dd, hr,plev, yr=2014, mth=12):
    ...
    minlat = 0;maxlat = 60; minlon = 85; maxlon = 175;
...
def plot_data(psi_list,avg_psi,outfile,dt,md,TT,pos_id,plev,dd,hr,dtick=10):
    ...
    ensemble_members=np.arange(12)
    psi_levs = np.arange(-1e8,1e8,3e6)
    for em in ensemble_members:
...
def plot_avg_slines(avg_psi,outfile,md,TT,dd,hr,pos_id,dtick=10):
    ...
    ax = plt.gca()
    map_formatter(ax,labelsize=40,tick_base_x=dtick,tick_base_y=dtick)
...
def plot_sing_mem(psi,outfile,md,TT,dd,hr,pos_id,em0,dtick=10):
    ensemble_members = np.arange(12)
    psi_levs = np.arange(-1e8,1e8,2e6)
    for em in ensemble_members:
        dataname = '/nfs/a37/scjea/plot_scripts/tc_analysis/intensity_track/track_data/tcver_glm_{0:04d}_{1:02d}Z_en{2:02d}.npz'.format(md,TT,em)
    ...
    minlat = -10;maxlat = 30; minlon = 105; maxlon = 180;
    ax = plt.gca()
    map_formatter(ax,labelsize=36,tick_base_x=dtick,tick_base_y=dtick)
    ax.set_xlim([minlon,maxlon]); ax.set_ylim([minlat,maxlat])
...
def main():
    ...
    vt_day = [3,4,5,6,7,8,9]; vt_hr = [0, 6, 12, 18]
    plevs = [200]
    #### Load cubes:
    for md in monthday:
        for TT in times:
            ...
            for dd in vt_day:
                for hr in vt_hr:
                    if dd==(md-1200) and hr < TT:
                        continue
                    for plev in plevs:
                        outfile = '/nfs/a37/scjea/model_runs/Hagupit/u-ap087/plots/glm/1203_12Z/streamlines/comp_streamlines_850-200hPa_{0:02d}_{1:02d}Z.png'.format(dd,hr)
                        ...
                        psi_list = load_ens_members(fpath,dd,hr,plev,yr=2014,mth=12)
                        avg_psi = avg_cubes(psi_list)
                        #era_psi = load_era_data(avg_psi,dd,hr,plev)
                        #plot_sing_mem(psi_list[9],outfile,md,TT,dd,hr,pos_id,9,dtick=10)
