...
def plot_data(data_list,avg_data,era_data,outfile,md,TT,pos_id,dtick=5):
    #brewer_cmap = mpl_cm.get_cmap('brewer_OrRd_09')
    ensemble_members=np.arange(12)
    cmap = mpl_cm.get_cmap('brewer_Accent_08')
    
    for em in ensemble_members:
        dataname = '/nfs/a37/scjea/plot_scripts/tc_analysis/intensity_track/track_data/tcver_4p4_{0:04d}_{1:02d}Z_en{2:02d}.npz'.format(md,TT,em)
    ...
    for em in ensemble_members:
        ...
        cube_plt = iplt.contourf(cube,levels=cube_levs,cmap=cmap,norm=norm)
...
def main():
    ...
    stash = 'm01s16i222';
    era_name = 'air_pressure_at_sea_level';
    #### Load cubes:
    for md in monthday:
        for TT in times:
            pos_id = 0
            for dd in vt_day:
                for hr in vt_hr:
                    if dd==(md-1200) and hr < TT:
                        continue
                    for plev in plevs:
                        outfile = '/nfs/a37/scjea/model_runs/Hagupit/u-ap087/plots/4p4/{0:04d}_{1:02d}Z/ensemble_plots/slp_anom_{2:02d}_{3:02d}Z_{4:03d}hPa.png'.format(md,TT,dd,hr,plev)
                        fpath = '/nfs/a37/scjea/model_runs/Hagupit/u-ap087/data/4p4/{0:04d}_{1:02d}Z/tctracker/tcver_4p4_{0:04d}_{1:02d}Z_em'.format(md,TT)
                        ...
                        era_data = load_era_data(era_name,avg_data,dd,hr,plev)
                        plot_data(data_list,avg_data,era_data,outfile,md,TT,pos_id)
                        pos_id = pos_id + 1
