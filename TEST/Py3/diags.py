    ...
    def loop_olr(self, time):
        ...
        for i in range(18):
            ...
            with open(self.olrhome + 'late{0:02}.pkl'.format(i), "rb") as pkl:
                latt = np.asarray(pickle.load(pkl, encoding='bytes'))
            with open(self.olrhome + 'lone{0:02}.pkl'.format(i), "rb") as pkl:
                lonn = np.asarray(pickle.load(pkl, encoding='bytes'))
...
class DiagPlotter(object):
    ...
    def plot_ens(self, ax_ws, fpat_ens, contraints, labels):
        ...
        Returns:
            approriate subplot
        """
        d_file = fpat_ens[0] + '{0:02n}.pp'.format(fpat_ens[1])
        ...
        ax_ws.annotate('Em {0:02n}'.format(fpat_ens[1]),
                       xy=(0.97, 0.03),
